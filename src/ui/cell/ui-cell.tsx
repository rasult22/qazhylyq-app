...
const UICell: React.FC<UICellProps> = ({
  ...
  return (
    <button
      ...
      className={`flex items-center text-[#F9F7F4] w-[90vw] text-left shadow-lg active:opacity-[0.9] transition-all active:scale-[1.01] font-bold py-[14px] px-5 rounded-[16px] ${className}`}
      onClick={onClick}
    >
      {children}
      {isLink ? (
